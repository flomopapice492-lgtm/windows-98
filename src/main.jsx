...
const eras = Object.fromEntries(Array.from({ length: 22 }, (_, i) => {
  const year = 1998 + i
  const windows = year < 2000 ? 'Windows 98' : year < 2001 ? 'Windows 2000' : year < 2006 ? 'Windows XP' : year < 2009 ? 'Windows Vista' : year < 2012 ? 'Windows 7' : year < 2015 ? 'Windows 8.1' : 'Windows 10'
  ...
  return [year, { name: windows, browser, color: year < 2006 ? '#008080' : '#0078d7', wallpaper: year < 2001 ? 'clouds' : year < 2006 ? 'bliss' : year < 2012 ? 'vista' : 'windows10' }]
}))
...
function App() {
  ...
  return <main className={`desktop wallpaper-${era.wallpaper}`} style={{ '--accent': era.color }} onClick={() => menu && setMenu(false)}>
    <header className="era-bar"><strong>Windows 98 Time Machine</strong><label>Era: <select value={year} onChange={e => { setYear(Number(e.target.value)); setWindow(null) }}>{years.map(y => <option key={y}>{y}</option>)}</select></label><span>{era.name} · {era.browser}</span></header>
    ...
    {window && <AppWindow title={windowTitle(window, era)} onClose={() => setWindow(null)}>
      {window === 'flash' && <FlashPlayer year={year} />}
      {window === 'internet' && <Internet browser={era.browser} year={year} onFlash={() => open('flash')} />}
      {window === 'computer' && <FileExplorer />}
      {window === 'documents' && <FileExplorer documents />}
      {window === 'recycle' && <div className="empty">The Recycle Bin is empty.</div>}
    </AppWindow>}
    <footer className="taskbar" onClick={e => e.stopPropagation()}><button className="start" onClick={() => setMenu(!menu)}>▣ Start</button><span className="task">{window ? windowTitle(window, era) : 'Ready'}</span><span className="clock">{year}/09/20&nbsp; 12:00 PM</span>{menu && <div className="start-menu"><div className="side">Windows<br />98</div><div className="menu-items"><button onClick={() => open('internet')}>🌐 {era.browser}</button><button onClick={() => open('flash')}>⚡ Macromedia Flash Player / Ruffle</button><button onClick={() => open('computer')}>🖥️ {year < 2002 ? 'My Computer' : 'This PC'}</button><hr /><button onClick={() => open('documents')}>📁 Documents</button><button onClick={() => open('recycle')}>🗑️ Recycle Bin</button></div></div>}</footer>
  </main>
}
...
function FlashPlayer({ year }) {
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      const instance = ruffle.createPlayer()
      ...
      instance.load({ url }).catch(() => setStatus('Unable to load this SWF. It may require unavailable legacy features or CORS access.'))
    }
    if (window.RufflePlayer) start()
    else {
  ...
  const loadUrl = () => { if (!player.current || !url.startsWith('https://')) return setStatus('For safety, remote Flash URLs must use HTTPS.'); setStatus('Loading…'); player.current.load({ url }).catch(() => setStatus('This SWF could not be loaded. Check the URL and CORS permissions.')) }
  const loadFile = event => { const file = event.target.files?.[0]; if (!file) return; setFileName(file.name); setStatus('Loading local SWF…'); player.current?.load({ buffer: new Uint8Array(awaitFile(file)) }).catch(() => setStatus('This local SWF could not be played.')) }
  const awaitFile = file => { const reader = new FileReader(); reader.readAsArrayBuffer(file); return reader.result || new ArrayBuffer(0) }
  return <div className="flash"><div className="flash-toolbar">File&nbsp;&nbsp; View&nbsp;&nbsp; Control&nbsp;&nbsp; Help <span>Macromedia Flash Player 6 · Ruffle</span></div><div className="flash-stage" ref={host}><div className="loading">{status}</div></div><div className="flash-controls"><input value={url} onChange={e => setUrl(e.target.value)} aria-label="SWF URL" /><button onClick={loadUrl}>Load URL</button><label className="file-button">Open SWF<input type="file" accept=".swf,application/x-shockwave-flash" onChange={loadFile} /></label></div><p className="notice">{status} · Current file: {fileName} · Ruffle is an open-source Flash emulator; original proprietary plug-ins are not included.</p><p className="era-note">Flash content may fail when a website blocks cross-origin requests. Use downloaded SWF files you are allowed to play.</p></div>
}
